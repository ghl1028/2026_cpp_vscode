...
void update(const std::string& name, int score, int solved) {
    ...
    if(board.count(name) > 0) board[name] = {name, score, solved} ;
}

std::vector<std::string> qualifiers(int cutoff) {
    // cutoff 이상의 점수를 가진 참가자 이름을 이름 오름차순으로 반환합니다.
    std::vector<std::string> result ;
    for(auto& [name, player] : board) {
        if(player.score >= cutoff) result.push_back(name) ;
    }
    return result ;
}
...
void print_board() {
    // 순위표를 이름 오름차순
    for(auto& [name, player] : board) {
        std::cout << player.name << " " << player.score << " " << player.solved << '\n' ;
    }
}